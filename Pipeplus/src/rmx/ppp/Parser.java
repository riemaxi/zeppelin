...
public class Parser {
    ...
    protected String nextToken(StringBuffer token, InputStreamReader reader){
        if (leftBehind != C.PARSER_SYMBOL_LEFT_BEHIND){
            String next = Character.toString((char)leftBehind);
            leftBehind = C.PARSER_SYMBOL_LEFT_BEHIND;
            return next;
        }
    ...
    protected void skipComment(InputStreamReader reader, String open, String close){
        try{
            int ch = reader.read();
            char commentclose = close.charAt(0);
            while(ch != -1 && ch != commentclose)
                ch = reader.read();
            
            if (ch == -1 && !open.equals(C.PARSER_COMMENT_OPEN))
    ...
    public void parse(InputStreamReader reader, Builder builder){
        ...
        level = C.PARSER_SYMBOL_LEFT_BEHIND;
         for(   nxtoken = nextToken(token, reader); 
                ...
                switch(nxtoken){
                    ...
                    case C.PARSER_COMMENT_OPEN : skipComment(reader, C.PARSER_COMMENT_OPEN, C.PARSER_COMMENT_CLOSE); break;
                    case C.PARSER_COMMENT_HASH_OPEN : skipComment(reader, C.PARSER_COMMENT_HASH_OPEN, C.PARSER_COMMENT_HASH_CLOSE);break;
